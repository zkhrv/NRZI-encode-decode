...
def plot_encoded_signal(encoded_signal): #
    ...
    for bit in encoded_signal: #  Перебираем биты закодированного сигнала
...
def nrzi_encode_decode(): # метод для кодирования/декодирования
    ...
    print("Кодированный сигнал: ", encoded_signal) # выводим кодированный сигнал 
    print("Время кодирования:", "{:.8f}".format(time)) # выводим время кодирования

    # Декодирование
    decoded_bits = '' # инициализируем переменную decoded_signal пустой строкой, в нее будут добавляться декодированные символы
    signal = '1' # задаем значение переменной signal, которое используется для декодирования сигнала
    for i in range(0, len(encoded_signal)): # Перебираем все биты в закодированном сигнале
    ...
    print("Декодированная строка: ", decoded_string) # Выводим декодированную строку
